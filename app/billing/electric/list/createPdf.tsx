...
export const createPdf = async () => {
  ...
  makeItem(page, " 2023/8/1 ", 10, customFont, 480, height - 100, 100, 12, 0);

  // 住所欄
  makeItem(page, " 郵便番号 ", 10, customFont, 30, height - 130, 50, 12, 0.1);
  makeItem(page, " 000-0111 ", 10, customFont, 80, height - 130, 200, 12, 0);

  makeItem(page, " 住所 ", 10, customFont, 30, height - 147, 50, 12, 0.1);
  makeItem(page, " 東京都新宿区 ", 10, customFont, 80, height - 147, 200, 12, 0);

  makeItem(page, " お客様名 ", 10, customFont, 30, height - 164, 50, 12, 0.1);
  makeItem(page, " 株式会社～ 御中 ", 10, customFont, 80, height - 164, 200, 12, 0);
  ...
  makeItem(page, " 　　　　　　　　　　　　　　　　　　　　　　　　　7,041 円 ", 10, customFont, 230, height - 220, 350, 12, 0);

  makeItem(page, " (内消費税相当額)  ", 10, customFont, 30, height - 237, 200, 12, 0.1);
  makeItem(page, " 　　　　　　　　　　　　　　　　　　　　　　　　　  640 円 ", 10, customFont, 230, height - 237, 350, 12, 0);
  ...
  makeItem(page, " SYS0000 ", 10, customFont, 130, height - 275, 150, 12, 0);

  makeItem(page, " 供給地点特定番号 ", 10, customFont, 30, height - 292, 100, 12, 0.1);
  makeItem(page, " ～ ", 10, customFont, 130, height - 292, 150, 12, 0);
  
  makeItem(page, " 供給地点地点名 ", 10, customFont, 30, height - 309, 100, 12, 0.1);
  makeItem(page, " 株式会社～ ", 10, customFont, 130, height - 309, 150, 12, 0);

  makeItem(page, " ご契約名義 ", 10, customFont, 30, height - 326, 100, 12, 0.1);
  makeItem(page, " 株式会社～ ", 10, customFont, 130, height - 326, 150, 12, 0);

  makeItem(page, " 供給地点住所 ", 10, customFont, 30, height - 343, 100, 12, 0.1);
  makeItem(page, " 東京都～ ", 10, customFont, 130, height - 343, 150, 12, 0);

  makeItem(page, " ご契約料金メニュー ", 10, customFont, 30, height - 360, 100, 12, 0.1);
  makeItem(page, " syshan電気(プランB) ", 10, customFont, 130, height - 360, 150, 12, 0);

  makeItem(page, " ご契約電流 ", 10, customFont, 30, height - 377, 100, 12, 0.1);
  makeItem(page, " 50 A ", 10, customFont, 130, height - 377, 150, 12, 0);

  makeItem(page, " ご使用期間 ", 10, customFont, 30, height - 394, 100, 12, 0.1);
  makeItem(page, " 2023/6/3 ～ 2023/7/2 ", 10, customFont, 130, height - 394, 150, 12, 0);

  makeItem(page, " ご使用日数 ", 10, customFont, 30, height - 411, 100, 12, 0.1);
  makeItem(page, " 30 / 29 日 ", 10, customFont, 130, height - 411, 150, 12, 0);

  makeItem(page, " 当月ご使用量 ", 10, customFont, 30, height - 428, 100, 12, 0.1);
  makeItem(page, " 299 kWh ", 10, customFont, 130, height - 428, 150, 12, 0);

  // お支払い方法欄
  makeItem(page, " お支払い方法   ", 10, customFont, 300, height - 290, 100, 12, 0.1);
  makeItem(page, " 口座振込  ", 10, customFont, 400, height - 290, 180, 12, 0);
  ...
  makeItem(page, "  燃料費調整額 ", 10, customFont, 30, height - 551, 120, 12, 0.1);
  makeItem(page, " 　　 ", 10, customFont, 150, height - 551, 80, 12, 0);
  ...
  makeItem(page, " 　　  ", 10, customFont, 350, height - 551, 40, 12, 0);
  makeItem(page, " 　　　 ", 10, customFont, 390, height - 551, 80, 12, 0);
  ...
  download(pdfBytes, "billing_20230715.pdf", "application/pdf");
}